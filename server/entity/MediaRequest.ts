...
export class MediaRequest {
  public static async request(
    ...
    // Music requests use MusicBrainz IDs, not TMDB
    if (requestBody.mediaType === MediaType.MUSIC) {
      if (!requestBody.musicBrainzId) {
        throw new Error('musicBrainzId is required for music requests.');
      }
      ...
      if (!media) {
        media = new Media({
          tmdbId: 0, // Not used for music but column is not nullable
          musicBrainzId: requestBody.musicBrainzId,
      ...
      // Check for duplicate music requests
      if (media.requests && media.requests.length > 0) {
        const activeDuplicate = media.requests.find(
          (r) =>
      ...
      const request = new MediaRequest({
        ...
        musicBrainzId: requestBody.musicBrainzId,
        artistMusicBrainzId: requestBody.artistMusicBrainzId,
        artistName: requestBody.artistName,
        albumTitle: requestBody.albumTitle,
        metadataProfileId: requestBody.metadataProfileId,
      });
  ...
  // Music-specific: the MusicBrainz release group ID for the requested album
  @Column({ nullable: true, type: 'varchar' })
  public musicBrainzId?: string | null;

  // Music-specific: the MusicBrainz artist ID
  @Column({ nullable: true, type: 'varchar' })
  public artistMusicBrainzId?: string | null;

  // Music-specific: stored for display and Lidarr submission
  @Column({ nullable: true, type: 'varchar' })
  ...
  // Music-specific: Lidarr metadata profile
  ...
  static async sendNotification(
    ...
    try {
      ...
      if (entity.type === MediaType.MOVIE) {
        ...
        // Music notifications use the stored artist/album names from the request
        const albumTitle = entity.albumTitle ?? 'Unknown Album';
        ...
        notificationManager.sendNotification(type, {
          ...
          notifyUser: notifyAdmin ? undefined : entity.requestedBy,
          event,
          subject: `${albumTitle} - ${artistName}`,
          message: `Music album request: ${albumTitle} by ${artistName}`,
          image: undefined,
        });
