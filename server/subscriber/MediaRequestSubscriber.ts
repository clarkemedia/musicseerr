...
export class MediaRequestSubscriber implements EntitySubscriberInterface<MediaRequest> {
    ...
    if (
      ...
      try {
        ...
        let lidarrSettings = settings.lidarr.find(
          (lidarr) => lidarr.isDefault && !lidarr.is4k
        );
        ...
        const metadataProfileId =
          entity.metadataProfileId ?? lidarrSettings.metadataProfileId;
        ...
        if (lidarrSettings.tagRequests) {
          const lidarrTags = await lidarr.getTags();
          let userTag = lidarrTags.find((v) =>
            v.label.startsWith(entity.requestedBy.id + '-')
          );
          if (!userTag) {
          ...
          if (userTag?.id && !tags.find((v) => v === userTag?.id)) {
            tags.push(userTag.id);
          }
        }
        ...
        if (media.status === MediaStatus.AVAILABLE) {
          logger.warn(
            'Music already exists, marking request as COMPLETED',
            {
              label: 'Media Request',
              requestId: entity.id,
              mediaId: entity.media.id,
        ...
        if (!entity.musicBrainzId || !entity.artistMusicBrainzId) {
          logger.error(
            'Music request missing MusicBrainz IDs, cannot send to Lidarr',
            {
              label: 'Media Request',
              requestId: entity.id,
        ...
        const lidarrAlbumOptions = {
          title: entity.albumTitle ?? 'Unknown Album',
          foreignAlbumId: entity.musicBrainzId,
          qualityProfileId: qualityProfile,
          metadataProfileId,
          rootFolderPath: rootFolder,
          tags,
          ...
          artist: {
            ...
            foreignArtistId: entity.artistMusicBrainzId,
            qualityProfileId: qualityProfile,
            metadataProfileId,
            rootFolderPath: rootFolder,
            tags,
  ...
  public async updateParentStatus(entity: MediaRequest): Promise<void> {
    ...
    // Music uses the same simple decline logic as movies
    if (
