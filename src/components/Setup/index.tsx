...
const messages = defineMessages('components.Setup', {
  ...
  confignavidrome: 'Configure Navidrome',
  setup: 'Setup',
  finish: 'Finish Setup',
  finishing: 'Finishing\u2026',
  continue: 'Continue',
...
const Setup = () => {
  ...
  const validateLibraries = useCallback(async () => {
    // Navidrome has no library concept — consider setup complete once logged in
    if (mediaServerType === MediaServerType.NAVIDROME) {
      setMediaServerSettingsComplete(true);
      return;
    }

    try {
      const endpointMap: Partial<Record<MediaServerType, string>> = {
        ...
        <div className="mt-10 w-full rounded-md border border-gray-600 bg-gray-800/50 p-4 text-white">
          {currentStep === 1 && (
            <div className="flex flex-col items-center pb-6">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                <div className="flex flex-col divide-y divide-gray-600 rounded border border-gray-600 py-2">
                ...
                <div className="flex flex-col divide-y divide-gray-600 rounded border border-gray-600 py-2">
                  <div className="mb-2 flex flex-1 items-center justify-center px-2 py-2 text-xl font-bold text-white">
                    Navidrome
                  </div>
                  <div className="px-2 pt-2">
                    <button
                      onClick={() => {
                        setMediaServerType(MediaServerType.NAVIDROME);
                        setCurrentStep(2);
                      }}
                      className="button-md relative z-10 inline-flex h-full w-full items-center justify-center rounded-md border border-gray-600 bg-transparent px-4 py-2 text-sm font-medium leading-5 text-white transition duration-150 ease-in-out hover:z-20 hover:border-gray-200 focus:z-20 focus:border-gray-100 focus:outline-none active:border-gray-100"
                    >
                      {intl.formatMessage(messages.confignavidrome)}
                    </button>
                  </div>
                </div>
              </div>
          ...
          {currentStep === 3 && (
            <div className="p-2">
              ...
              ) : mediaServerType === MediaServerType.NAVIDROME ? (
                <div className="py-4 text-center text-gray-300">
                  Navidrome is configured. No library sync needed.
                </div>
              ) : (
